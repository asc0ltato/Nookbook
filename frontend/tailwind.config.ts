import type { Config } from "tailwindcss";

export default {
  darkMode: ["class"],
  content: [
    "./src/pages/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/components/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/app/**/*.{js,ts,jsx,tsx,mdx}",
  ],
  prefix: "",
  theme: {
    container: {
      center: true,
      padding: "2rem",
      screens: {
        "2xl": "1400px",
      },
    },
    extend: {
      colors: {
        border: "hsl(var(--border))",
        input: "hsl(var(--input))",
        ring: "hsl(var(--ring))",
        background: "hsl(var(--background))",
        foreground: "hsl(var(--foreground))",
        primary: {
          DEFAULT: "hsl(var(--primary))",
          foreground: "hsl(var(--primary-foreground))",
        },
        secondary: {
          DEFAULT: "hsl(var(--secondary))",
          foreground: "hsl(var(--secondary-foreground))",
        },
        destructive: {
          DEFAULT: "hsl(var(--destructive))",
          foreground: "hsl(var(--destructive-foreground))",
        },
        muted: {
          DEFAULT: "hsl(var(--muted))",
          foreground: "hsl(var(--muted-foreground))",
        },
        accent: {
          DEFAULT: "hsl(var(--accent))",
          foreground: "hsl(var(--accent-foreground))",
        },
        popover: {
          DEFAULT: "hsl(var(--popover))",
          foreground: "hsl(var(--popover-foreground))",
        },
        card: {
          DEFAULT: "hsl(var(--card))",
          foreground: "hsl(var(--card-foreground))",
        },
      },
// tailwind.config.ts
fontFamily: {
  // Cormorant Garamond - elegant serif for display and headings
  display: ['var(--font-display)', 'Cormorant Garamond', 'ui-serif', 'Georgia', 'serif'],
  serif: ['var(--font-display)', 'Cormorant Garamond', 'ui-serif', 'Georgia', 'serif'],
  
  // Montserrat - modern geometric sans for headings
  heading: ['var(--font-heading)', 'Montserrat', 'system-ui', 'sans-serif'],
  montserrat: ['var(--font-heading)', 'Montserrat', 'system-ui', 'sans-serif'],
  
  // Manrope - clean geometric sans for body text
  body: ['var(--font-body)', 'Manrope', 'system-ui', 'sans-serif'],
  sans: ['var(--font-body)', 'Manrope', 'system-ui', 'sans-serif'],
  manrope: ['var(--font-body)', 'Manrope', 'system-ui', 'sans-serif'],
},
      spacing: {
        'header': 'var(--header-height)',
        'search-bar': 'var(--search-bar-height)',
      },
      borderRadius: {
        lg: "var(--radius)",
        md: "calc(var(--radius) - 2px)",
        sm: "calc(var(--radius) - 4px)",
      },
      keyframes: {
        "accordion-down": {
          from: {
            height: "0",
          },
          to: {
            height: "var(--radix-accordion-content-height)",
          },
        },
        "accordion-up": {
          from: {
            height: "var(--radix-accordion-content-height)",
          },
          to: {
            height: "0",
          },
        },
      },
      animation: {
        "accordion-down": "accordion-down 0.2s ease-out",
        "accordion-up": "accordion-up 0.2s ease-out",
      },
    },
  },
  plugins: [require("tailwindcss-animate")],
} satisfies Config;
