...
export const HotelCard = ({
  ...
  const { data: favoritesResponse } = useQuery({
    ...
    enabled: isAuthenticated && authUserIdOk && !isElevatedUser,
  });

  const favorites = (favoritesResponse?.data as any[] | undefined) || [];
  const hotelIdNum = typeof hotel.id === "string" ? parseInt(hotel.id, 10) : Number(hotel.id);
  const favoriteHotelId = (f: any) => Number(f?.id ?? f?.Id ?? 0);
  const isFavorite = favorites.some((f: any) => favoriteHotelId(f) === hotelIdNum);

  const favoriteMutation = useMutation({
    mutationFn: (action: "add" | "remove") => {
      if (!authUserIdOk) throw new Error("Нет пользователя");
      const hid =
        typeof hotel.id === "string" ? parseInt(hotel.id, 10) : Number(hotel.id);
  ...
  const handleCardClick = () => {
    if (onClick) {
      ...
      window.open(`/hotels/${slug}/${hotel.id}`, '_blank');
    }
  };
  ...
  return (
    <Card 
      className={`cursor-pointer transition-all duration-300 hover:shadow-2xl hover:shadow-primary/10 hover:border-primary/30 border group hover:scale-[1.01] overflow-hidden bg-gradient-to-br from-card to-card/95 backdrop-blur-sm ${
        isHighlightedOnMap ? "border-primary ring-2 ring-primary/30" : "border-border/50"
      }`}
      onClick={handleCardClick}
    >
      <CardContent className="p-0">
        <div className="flex gap-0 h-[300px] w-[650px]">
          <div className="relative flex-shrink-0 w-56 overflow-hidden rounded-l-lg group-hover:scale-105 transition-transform duration-500 bg-gradient-to-br from-gray-200 to-gray-300 self-stretch">
            <ImageCarousel
              images={imageUrls}
              alt={hotel.name}
              sizes="300px"
            ...
            {isAuthenticated && !isElevatedUser && (
              <button
                ...
                className="absolute top-4 right-4 p-2.5 rounded-full bg-white/95 backdrop-blur-md hover:bg-white hover:scale-110 active:scale-95 transition-all duration-200 shadow-xl z-10 hover:shadow-2xl"
              >
                <Heart 
                  className={cn(
          ...
          <div className="flex-1 p-7 flex flex-col justify-between bg-gradient-to-br from-card via-card/98 to-card/95">
            <div>
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1 pr-4">
                  <h3 className="text-2xl font-serif font-semibold mb-2.5 group-hover:text-primary transition-colors duration-300 truncate bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text">
                    {hotel.name}
                  </h3>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    {hotel.stars === 0 && (
                      <span className="text-sm text-muted-foreground">Без звезд</span>
                    )}
                  </div>
                </div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {hotel.amenities.slice(0, 3).map((amenity) => (
                  <Badge key={amenity} variant="secondary" className="text-xs px-3 py-1.5 hover:bg-secondary/90 hover:scale-105 transition-all duration-200 font-medium shadow-sm border border-border/50">
                    {amenity}
                  </Badge>
                ))}
                {hotel.amenities.length > 3 && (
                  <Badge variant="secondary" className="text-xs px-3 py-1.5 font-medium shadow-sm border border-border/50">
                    +{hotel.amenities.length - 3}
                  </Badge>
                )}
              </div>
            ...
            <div className="border-t border-border/30 pt-4 flex flex-row items-end justify-between w-full gap-3">
              <div className="text-left min-w-0">
                <div className="text-3xl font-heading font-extrabold text-primary mb-1 group-hover:scale-105 transition-transform duration-300 bg-gradient-to-r from-primary to-primary/80 bg-clip-text text-transparent">
                  {hotel.price.toLocaleString()} BYN
                </div>
                <p className="text-sm text-muted-foreground font-medium">за 1 сутки</p>
              </div>
              {onShowOnMap && hotel.latitude && hotel.longitude && (
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  className={cn(
                    "h-9 px-4 rounded-lg border-border/60 bg-background/80",
                    "hover:bg-primary hover:text-primary-foreground",
                    "transition-all duration-200 shrink-0"
                  )}
                  onClick={(e) => {
                    e.stopPropagation();
                    onShowOnMap(String(hotel.id));
                  }}
                >
