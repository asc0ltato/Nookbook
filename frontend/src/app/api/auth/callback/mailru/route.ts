...
export async function GET(request: NextRequest) {
  ...
  if (!state) {
    return NextResponse.redirect(`${baseUrl}/?error=no_state`);
  }
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/auth/mailru/callback`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Backend error: ${response.status}. Endpoint /auth/mailru/callback может не существовать. ${text || 'Пустой ответ'}`);
    }

    if (!text || text.trim() === '') {
      throw new Error('Empty response from backend. Endpoint /auth/mailru/callback не реализован на backend.');
    }
